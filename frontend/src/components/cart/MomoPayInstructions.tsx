...
export default function MomoPayInstructions({
  ...
  return (
    ...
    >
      <p className="text-sm font-semibold text-dark">{t("momo.title")}</p>
      <p className="mt-1 text-xs text-muted">
        {t("momo.dialHint")}
      </p>

      <div className="mt-3 flex items-center justify-between gap-3">
        <span className="text-xs text-muted">{t("momo.amount")}</span>
      ...
      {orderReference && (
        <p className="mt-2 text-xs text-muted">
          {t("momo.useRef", { ref: orderReference })}
        </p>
      )}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        >
          {copied ? t("momo.copied") : t("momo.copy")}
        </button>
        <a
